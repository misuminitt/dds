-- DDS.lua (wrapped for loadstring(... )() usage)
-- If you want it to auto-run when required directly, keep calling the returned function.

return function()
    -- BEGIN ORIGINAL DDS SCRIPT (kept mostly intact)
    ...
    local function buildGUI()
        ...
        pcall(function()
            local pg = player:FindFirstChildOfClass("PlayerGui")
            if pg then local old = pg:FindFirstChild(DDS.GUI_NAME); if old then old:Destroy() end end
        end)
    ...
    -- Watchdog (non-intrusive)
    RunService.Heartbeat:Connect(function()
        local pg = player:FindFirstChildOfClass("PlayerGui")
        if pg and not pg:FindFirstChild(DDS.GUI_NAME) then
        end
    end)
    -- END ORIGINAL DDS SCRIPT

    -- expose DDS object if needed
    _G._DDS_PRIVATE = DDS

    -- optionally auto-start? comment/uncomment next line depending on preference
    -- DDS:start()

    -- RETURN: allow loadstring(... )() to run the script and get this function's return (nil)
end
